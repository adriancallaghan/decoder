...
[-clubs]             Used for filtering Club names, usage is -clubs followed by equals and comma delimited list of names e.g leicester,birmingham (other filtering should be queried via the db) by default all clubs are included
...
if ($in=='help' || $in=='man' || $in=='' || $out==''){
	...
	message('[-clubs]    Used for filtering Club names, usage is -clubs followed by equals and comma delimited list of names e.g leicester,birmingham (other filtering should be queried via the db) by default all clubs are included');
...
message("Clubs: ".(empty($clubs) ? 'ALL' : implode(',',$clubs)));
message("Fields: ".(empty($includes) ? 'ALL' : implode(',',$includes)));
...
while (($line = fgetcsv($file)) !== FALSE) {
	...
	// promo filter
	if (!empty($promoCodes) && isset($data[$lineNo]['hdnPromo']) && !in_array($data[$lineNo]['hdnPromo'], $promoCodes)){
		$filtered = true;
		$feedbackChar = 'P';
	...
	// club filter
	if (!empty($clubs) && isset($data[$lineNo]['sltClub']) && !in_array($data[$lineNo]['sltClub'], $clubs)){
		$filtered = true;
		$feedbackChar = 'C';
		$data[$lineNo]['exclusion'] = "Club";
	}
